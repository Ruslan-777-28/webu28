        ...
        if (esoIndex === -1) {
            ...
            for (const newSub of newSubcategories) {
                ...
                if (subIndex === -1) {
                    ...
                    // Update missing properties safely
            ...
            // Sort them if needed, or leave as is. Let's sort them.
            existingCat.subcategories.sort((a, b) => (a.sortOrder ?? 999) - (b.sortOrder ?? 999));
        }

        await ref.update({ categories: data.categories });
        
        return NextResponse.json({ success: true, message: 'Successfully seeded Esotericism taxonomy!', categories: data.categories });
    } catch (error: any) {
