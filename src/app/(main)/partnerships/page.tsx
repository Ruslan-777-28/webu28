...
export default function PartnershipsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.email || !formData.company || !formData.partnershipType || !formData.message) {
          ...
          <div className="text-center max-w-2xl mx-auto">
            ...
            {isLoading ? (
              ...
              <div className="text-lg text-muted-foreground/80 font-light leading-relaxed whitespace-pre-wrap">
                {settings?.partnershipsIntro || 'Ми відкриті до синергії. Якщо ви поділяєте наші цінності та маєте пропозицію щодо стратегічного партнерства, заповніть форму нижче.'}
              </div>
            )}
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-2">
                    <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground">Ім'я представника *</label>
                    <input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                   <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground">Компанія / Організація *</label>
                   <input 
                ...
                <div className="space-y-2">
                   <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground">Тип партнерства *</label>
                   <select 
                     name="partnershipType" value={formData.partnershipType} onChange={handleChange} required
              ...
              <div className="space-y-2">
                 <label className="text-xs uppercase tracking-wider font-bold text-muted-foreground">Пропозиція *</label>
                 <textarea 
                   ...
                   placeholder="Деталі вашої пропозиції або формат співпраці..."
                 ></textarea>
              </div>
              ...
              <div className="pt-4 flex justify-end">
                  ...
                  >
                    {isSubmitting ? 'Надсилання...' : 'Надіслати пропозицію'}
                  </button>
              </div>
            </form>

            {!isLoading && settings?.showDirectEmails && settings?.partnershipsEmail && (
               <div className="mt-12 pt-8 border-t border-border/30 text-center">
                  <p className="text-sm text-muted-foreground/60 mb-2">Direct email для партнерств:</p>
                  <a href={`mailto:${settings.partnershipsEmail}`} className="text-foreground tracking-wide font-medium hover:text-accent transition-colors">
                    {settings.partnershipsEmail}
                  </a>
               </div>
            )}
          </div>
