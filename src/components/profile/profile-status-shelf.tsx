...
import { Mic, Shield, MessageCircle, Repeat, TrendingUp, Crown, Star } from 'lucide-react';
...
export function ProfileStatusShelf({ subcategoryName }: { subcategoryName?: string }) {
    ...
    // The selector handles normalization, joining definitions, filtering, and priority sorting.
    const targetUserId = getDemoTargetUserId();
    const awards = getProfileAwardsForSubcategory(targetUserId, subcategoryName).slice(0, 3);

    if (awards.length === 0) {
        return (
            <div className="w-full mt-6 pt-4 border-t border-muted/10">
                <div className="flex items-center gap-2.5 py-3 px-4 rounded-lg bg-background border border-muted/20 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5 items-stretch">
                {awards.map(award => {
                    ...
                    return (
                        <div 
                            key={award.id} 
                            className="flex flex-col items-start gap-2 p-2.5 rounded-xl bg-background border border-muted/20 shadow-sm hover:border-accent/30 hover:shadow-md transition-all group overflow-hidden"
                        >
